...
import { ClipboardPenIcon, GraduationCapIcon, XCircleIcon, CheckSquareIcon, ShieldAlertIcon, PrinterIcon, FileTextIcon, DownloadIcon } from '../../../Icons';
...
export const actionCards: { mode: InputMode; title: string; description: string; icon: React.FC<any> }[] = [
    ...
    { mode: 'violation_export', title: 'Export Pelanggaran', description: 'Unduh rekap pelanggaran kelas dalam format PDF atau Excel.', icon: DownloadIcon },
...
export const Step1_ModeSelection: React.FC<{ handleModeSelect: (mode: InputMode) => void }> = ({ handleModeSelect }) => (
    <div className="w-full max-w-7xl mx-auto space-y-10 animate-fade-in-up">
        <header className="relative overflow-hidden rounded-[2.5rem] bg-gradient-to-br from-indigo-600 via-purple-600 to-indigo-700 p-10 md:p-14 shadow-2xl shadow-indigo-500/20">
            ...
            <div className="relative flex flex-col md:flex-row items-center gap-8 md:gap-12 text-center md:text-left z-10">
                ...
                <div className="flex-1 space-y-3">
                    ...
                    <p className="text-lg md:text-xl text-indigo-100 leading-relaxed max-w-3xl mx-auto md:mx-0 font-light tracking-wide">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
            {actionCards.map((card, index) => (
                <div key={card.mode} onClick={() => handleModeSelect(card.mode)}
                    className="group relative overflow-hidden bg-white dark:bg-slate-900 backdrop-blur-xl rounded-3xl p-6 md:p-8 transition-all duration-500 hover:-translate-y-2 cursor-pointer border border-slate-200 dark:border-slate-700 hover:border-indigo-300 dark:hover:border-indigo-500/50 hover:shadow-2xl hover:shadow-purple-500/10 dark:hover:shadow-purple-500/20 shadow-lg"
                    style={{ animationDelay: `${index * 100}ms` }}>

                    <div className="absolute inset-0 bg-gradient-to-br from-purple-500/5 dark:from-purple-500/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>

                    <div className="relative z-10 flex flex-col items-center text-center space-y-4">
                        <div className="w-20 h-20 bg-gradient-to-br from-indigo-100 to-purple-100 dark:from-indigo-900/50 dark:to-purple-900/30 rounded-2xl flex items-center justify-center border border-indigo-200 dark:border-indigo-800/50 group-hover:scale-110 transition-transform duration-500 shadow-inner">
                            <card.icon className="w-10 h-10 text-indigo-600 dark:text-indigo-300 group-hover:text-indigo-700 dark:group-hover:text-white transition-colors duration-300 drop-shadow-md" />
                        </div>
                        <div>
                            <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-2 tracking-wide">{card.title}</h3>
