    ...
    >
        <div className="p-4 flex flex-col h-full gap-4">
            ...
            <div className="flex justify-between items-start">
                <div className="flex items-center gap-2 text-slate-400 bg-slate-800/50 px-2 py-1 rounded-lg border border-slate-700/50">
                    <ClockIcon className="w-3 h-3" />
                    <span className="text-[10px] sm:text-xs font-medium tracking-wide">{item.start_time} - {item.end_time}</span>
                </div>

                <DropdownMenu>
                    <DropdownTrigger className="w-7 h-7 flex items-center justify-center rounded-lg bg-slate-800/50 border border-slate-700/50 text-slate-400 hover:bg-slate-700 hover:text-white transition-colors">
            ...
            <div className="pt-2 border-t border-slate-800/50 flex items-center justify-between">
                <div className="flex items-center gap-1.5">
                    <div className={`w-1.5 h-1.5 rounded-full ${isOngoing ? 'bg-green-500 animate-pulse' : isPast ? 'bg-slate-600' : 'bg-indigo-500'}`}></div>
                    <span className={`text-[10px] font-medium ${isOngoing ? 'text-green-400' : 'text-slate-500'}`}>
            ...
            <div className="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8 space-y-8">
                ...
                <div className="space-y-6">
                            ...
                            {daysOfWeek.map((day) => {
                                ...
                                return (
                                    <button
                                        ...
                                        `}
                                    >
                                        <span className={`text-[10px] font-bold uppercase tracking-widest ${isSelected ? 'text-indigo-100' : 'text-slate-500'}`}>{day.substring(0, 3)}</span>
                    ...
                    {currentDaySchedule.length === 0 ? (
                        <div className="flex flex-col items-center justify-center py-16 text-center bg-[#0F172A] rounded-3xl border border-dashed border-slate-800">
                            ...
                            <h3 className="text-lg font-bold text-white mb-1">Kosong</h3>
                            <p className="text-slate-400 text-sm max-w-xs mx-auto mb-4">
                                Tidak ada jadwal untuk hari ini.
                            </p>
                            <Button onClick={() => handleOpenAddModal()} variant="outline" size="sm" className="rounded-full border-slate-700 text-slate-300 hover:bg-slate-800">
